...
Transform 展开、敌对阵营搜索、AI 上下文、写作 / ST 注入、探索子图隔离。
...
3. 正常对话即可——插件会在 `CHAT_COMPLETION_PROMPT_READY` 事件上拦截 Prompt，提取出场角色（当前角色卡名 + 消息中的 `@提及`），查询图谱上下文并注入（跳数取自项目 `writing_context` 设置，插件可通过 `?hop=` 覆盖）。
